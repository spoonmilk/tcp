    ...
    fn get_all_commands(&self) -> Vec<(String, CommandData)> {
        let mut custom_commands = vec![
            (
                "a".to_string(),
                CommandData {
                    handler: Self::wrap_host_handler(Self::a_handler),
                    num_args: NumArgs::Exactly(1),
                },
            ),
            (
                "c".to_string(),
                CommandData {
                    handler: Self::wrap_host_handler(Self::c_handler),
                    num_args: NumArgs::Exactly(2),
                },
            ),
            (
                "ls".to_string(),
                CommandData {
                    handler: Self::wrap_host_handler(Self::ls_handler),
                    num_args: NumArgs::Exactly(0),
                },
            ),
            (
                "s".to_string(),
                CommandData {
                    handler: Self::wrap_host_handler(Self::s_handler),
                    num_args: NumArgs::Exactly(2),
                },
            ),
            (
                "r".to_string(),
                CommandData {
                    handler: Self::wrap_host_handler(Self::r_handler),
                    num_args: NumArgs::Exactly(2),
                },
            ),
            (
                "sf".to_string(),
                CommandData {
                    handler: Self::wrap_host_handler(Self::sf_handler),
                    num_args: NumArgs::Exactly(3),
                },
            ),
            (
                "rf".to_string(),
                CommandData {
                    handler: Self::wrap_host_handler(Self::rf_handler),
                    num_args: NumArgs::Exactly(2),
                },
            ),
            (
                "cl".to_string(),
                CommandData {
                    handler: Self::wrap_host_handler(Self::cl_handler),
                    num_args: NumArgs::Exactly(1),
                },
            ),
        ];
...
impl HostRepl {
    ...
    pub fn run(mut self, ip_recver: Receiver<Packet>) -> () {
        //Can't be put in trait because of weird size issue
    ...
    pub fn a_handler(backend: &HostBackend, args: Vec<String>) -> () {
        //Sanititize input
        let port = if let Ok(port) = args[0].parse::<u16>() {
            port
        } else {
            return println!("Input port \"{}\" invalid", args[0]);
        };
    ...
    pub fn c_handler(backend: &HostBackend, args: Vec<String>) -> () {
        //Sanitize input
        let ip_addr = if let Ok(ip_addr) = args[0].parse::<Ipv4Addr>() {
            ip_addr
        } else {
            return println!("Input IP address \"{}\" invalid", args[0]);
        };
        let port = if let Ok(port) = args[1].parse::<u16>() {
            port
        } else {
            return println!("Input IP address \"{}\" invalid", args[1]);
        };
    ...
    pub fn ls_handler(backend: &HostBackend, _args: Vec<String>) -> () {
        ...
        for (sid, ent) in &*socket_table {
            let to_print = match ent {
                SocketEntry::Connection(ent) => format!(
                    "{sid:?}\t{}\t{}\t{}\t{}\t{:?}",
                    ent.src_addr.ip,
                    ent.src_addr.port,
                    ent.dst_addr.ip,
                    ent.dst_addr.port,
                    ent.state.read().unwrap()
                ),
                SocketEntry::Listener(ent) => format!(
                    "{sid:?}\t*\t\t{}\t*\t\t*\t{:?}",
                    ent.port,
                    ent.state.read().unwrap()
                ),
    ...
    pub fn s_handler(backend: &HostBackend, args: Vec<String>) -> () {
        //Sanitize input
        let sid = if let Ok(sid) = args[0].parse::<SocketId>() {
            sid
        } else {
            return println!("Input socket ID {} invalid", args[0]);
        };
    ...
    pub fn r_handler(backend: &HostBackend, args: Vec<String>) -> () {
        ...
        let bytes = if let Ok(bytes) = args[1].parse::<u16>() {
            bytes
        } else {
            return println!("Input number of bytes to read {} cannot parse to u16 - make sure it's less than 2^16", args[1]);
        };
        ...
        match backend.tcp_recieve(sid, bytes) {
            Ok(data) => {
                ...
                println!(
                    "Received {} bytes. As a string, they are:\n{}",
                    msg.len(),
                    msg
                )
            }
    ...
    pub fn sf_handler(backend: &HostBackend, args: Vec<String>) -> () {
        ...
        let filepath: PathBuf = {
            if let Ok(path) = Path::new(&args[0]).canonicalize() {
                ...
                if path.exists() && path.is_file() {
                    ...
                    return eprintln!(
                        "Input file path {} does not exist or is not a file",
                        args[0]
                    );
                }
            } else {
        ...
        let ip_addr = if let Ok(ip_addr) = args[1].parse::<Ipv4Addr>() {
            ip_addr
        } else {
            return eprintln!("Input IP address \"{}\" invalid", args[1]);
        };
        let port = if let Ok(port) = args[2].parse::<u16>() {
            port
        } else {
            return eprintln!("Input port \"{}\" invalid", args[2]);
        };
    ...
    pub fn rf_handler(backend: &HostBackend, args: Vec<String>) -> () {
        //Sanitize input
        let path = if let Ok(path) = Path::new(&args[0]).canonicalize() {
            path
        } else {
            return eprintln!("Path \"{}\" invalid", args[0]);
        };
        let port = if let Ok(port) = args[1].parse::<u16>() {
            port
        } else {
            return eprintln!("Input port \"{}\" invalid", args[1]);
        };
    ...
    fn receive_file(backend: HostBackend, path: PathBuf, port: u16) {
        ...
        let sid = backend
            .accept1(port)
            .expect("No listener socket with input port found...");
        backend
            .close(listen_sid)
            .expect("No listener at the sid we just got back from listen()"); //Close the listener socket
        let mut file = File::create(path).expect("Unable to open file with path: {path:?}");
        let mut total_bytes_read = 0;
        loop {
            let data = backend
                .tcp_recieve(sid, READ_CHUNK as u16)
                .expect("No socket at the sid we just got back from accept1()...");
            if data.len() == 0 {
                break;
            } //Should only happen if connection is closing
    ...
    pub fn cl_handler(backend: &HostBackend, args: Vec<String>) -> () {
        ...
        if let Err(e) = backend.close(sid) {
            println!("{}", e.to_string())
        };
    }
...
impl RouterRepl {
    ...
    fn ip_listen(ip_recver: Receiver<Packet>) -> () {
        loop {
            let pack = ip_recver
                .recv()
                .expect("Error receiving packet from IP Daemon");
