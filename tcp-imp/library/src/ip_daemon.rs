...
impl IPDaemon {
    ...
    pub fn run(mut self, recv_rchan: Receiver<CmdType>) -> () {
        ...
        if self.n_type == IPDaemonType::Router {
        ...
        if my_type == IPDaemonType::Router {
            let rip_periodic = Arc::clone(&listen_mutex);
            let timeout_check = Arc::clone(&listen_mutex);
            thread::spawn(move || IPDaemon::rip_go(rip_periodic));
            thread::spawn(move || IPDaemon::run_table_check(timeout_check));
        }

        thread::spawn(move || IPDaemon::repl_listen(repl_mutex, recv_rchan));

        //Listen for messages from interfaces and handle them
        IPDaemon::interface_listen(listen_mutex);
    }

    /// Listen for REPL commands to the node
    fn repl_listen(slf_mutex: Arc<Mutex<IPDaemon>>, recv_rchan: Receiver<CmdType>) -> () {
        loop {
    ...
    fn rip_go(slf_mutex: Arc<Mutex<IPDaemon>>) {
        loop {
    ...
    fn run_table_check(slf_mutex: Arc<Mutex<IPDaemon>>) {
        loop {
    ...
    fn interface_listen(slf_mutex: Arc<Mutex<IPDaemon>>) -> () {
        loop {
    ...
    fn forward_packet(&mut self, pack: Packet) -> Result<()> {
        ...
        if !IPDaemon::packet_valid(pack.clone()) {
            return Ok(());
        }
        let pack = IPDaemon::update_pack(pack);
    ...
    fn proper_interface(&self, dst_addr: &Ipv4Addr) -> Result<Option<(&String, Ipv4Addr)>> {
        ...
        loop {
            ...
            let netmask = IPDaemon::longest_prefix(self.forwarding_table.keys().collect(), dst_ip)?;
    ...
    fn process_packet(&mut self, pack: Packet) -> () {
        let src = IPDaemon::string_ip(pack.header.source);
        let dst = IPDaemon::string_ip(pack.header.destination);
