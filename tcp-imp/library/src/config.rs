...
pub fn initialize(config_info: IPConfig) -> Result<IPDaemon> {
    ...
    let n_type = match config_info.routing_mode {
        ...
        RoutingType::Static => IPDaemonType::Host,
        RoutingType::Rip => IPDaemonType::Router,
    };
    ...
    //Create and return IPDaemon
    //let IPDaemon = IPDaemon::new(n_type, interfaces, interface_reps, forwarding_table, rip_table); //PLACEHOLDER for now; FIX later
    let node = IPDaemon::new(n_type, interface_reps, forwarding_table, rip_table); //PLACEHOLDER for now; FIX later
    Ok(node)
}

fn add_static_routes(fwd_table: &mut RoutingTable, static_routes: Vec<StaticRoute>) -> () {
...
fn add_local_routes(fwd_table: &mut RoutingTable, interface_reps: &InterfaceTable) -> () {
...
fn add_toself_routes(fwd_table: &mut RoutingTable, interface_reps: &InterfaceTable) -> () {
    let interface_reps = interface_reps.values();
    for interface_rep in interface_reps {
