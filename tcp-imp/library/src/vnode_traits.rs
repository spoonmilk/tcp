...
pub trait VnodeIPDaemon {
    ...
    fn build(&self, pb: PacketBasis, prot_num: u8) -> Packet {
    ...
    fn send(&mut self, addr: String, msg: Vec<u8>, prot_num: u8) -> () {
        ...
        let (inter_rep, next_hop) = match self.proper_interface(&ip_addr) {
            ...
            Ok(None) => { // Sent to self, process
                let self_pack = self.build(pb, 0); // TODO: Update for non-test packets
                self.process_packet(self_pack);
                return;
            }
            Err(e) => {
                // Panicking shouldn't happen on IP level, just drop the packet
                eprintln!("Couldn't find a proper interface for packet, dropping. Error: {e:?}");
                return; 
            }
        };
        println!("Sending test packet to next hop: {}", next_hop);
        let send_pack = self.build(pb, prot_num);
        inter_rep
            .command(InterCmd::Send(send_pack, next_hop))
