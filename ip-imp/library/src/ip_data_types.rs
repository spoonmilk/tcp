...
impl Node {
        ...
        let (inter_rep, next_hop) = match self.proper_interface(&ip_addr) {
            ...
            Err(e) => { 
                ...
                panic!("\nForwarding table entry for address not found: {e:?}");
                                // (self.interface_reps.get("default"), Ipv4Addr::new(0,0,0,0));
                               }
        };
        println!("Sending packet to {}", ip_addr);
    ...
    fn send_rip(&mut self, dst: Ipv4Addr, command: u16) -> () {
        match command {
            ...
            2 => { // Send a routing
                let rip_resp_msg: RipMsg = table_to_rip(&mut self.forwarding_table, 2, &mut self.rip_neighbors, dst);
                println!("Successfully created a routing message");
                let ser_resp_rip: Vec<u8> = serialize_rip(rip_resp_msg.clone());
                println!("Serialized routing message, sending to {}", dst);
                self.send(dst.to_string(), ser_resp_rip, false);
                println!("Sent routing message");
            }
